...
static struct list ready_list[5]; // queue 5개 생성
...
/*age*/
void age(struct thread *t) 
{
	int 0<=age<=20; //age의 범위
	...
	if (age == 20) { //age가 20이 되면 우선순위 한 단계 상승
	age = 0;
	t->priority++;
		}
...
thread_tick (void)   
{
  ...
  int time_slice; // 우선순위마다 time_slice를 다르게 하기 위하여 변수 선언
...
/* Update statistics. */
  ...
  /* 우선순위에 따라 시간 할당 */
  if (t->priority == 0) time_slice = 2;
  if (t->priority == 1) time_slice = 3;
  if (t->priority == 2) time_slice = 4;
  if (t->priority == 3) time_slice = 5;
  if (t->priority == 4) time_slice = 6;
...
thread_yield (void)   
{
  ...
  if (t->priority == 1) {
	  if (cur != idle_thread)
		  list_push_back(&ready_list[1], &cur->elem);
  }
  if (t->priority == 2) {
	  if (cur != idle_thread)
		  list_push_back(&ready_list[2], &cur->elem);
  }
  if (t->priority == 3) {
	  if (cur != idle_thread)
		  list_push_back(&ready_list[3], &cur->elem);
  }
  if (t->priority == 4) {
	  if (cur != idle_thread)
		  list_push_back(&ready_list[4], &cur->elem);
  }
